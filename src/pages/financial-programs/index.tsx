...
import { getFinancialPrograms, deleteFinancialProgramById } from 'apiSdk/financial-programs';
import { FinancialProgramInterface } from 'interfaces/financial-program';

type ColumnType = ColumnDef<FinancialProgramInterface, unknown>;

interface FinancialProgramListPageProps {
...
export function FinancialProgramListPage(props: FinancialProgramListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<FinancialProgramInterface>>(
    () => `/financial-programs?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await deleteFinancialProgramById(id);
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: FinancialProgramInterface) => {
    if (hasAccess('financial_program', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/financial-programs/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/financial-programs/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('financial_program', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/financial-programs/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('financial_program', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="financial_program" />
        </Flex>

        {hasAccess('financial_program', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/financial-programs/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
...
export default compose(
  ...
  withAuthorization({
    ...
    entity: 'financial_program',
    operation: AccessOperationEnum.READ,
  }),
  withAppLayout(),
)(FinancialProgramListPage);
