...
type ColumnType = ColumnDef<BudgetInterface, unknown>;
...
export function BudgetListPage(props: BudgetListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<BudgetInterface>>(
    () => `/budgets?params=${JSON.stringify(params)}`,
    fetcher,
  );
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/budgets/view/${record.id}`} passHref legacyBehavior>
                <Button
