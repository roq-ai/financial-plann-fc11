...
import { getBudgets } from 'apiSdk/budgets';
import { BudgetInterface } from 'interfaces/budget';
import { getFinancialPolicies } from 'apiSdk/financial-policies';
import { FinancialPolicyInterface } from 'interfaces/financial-policy';
import { getFinancialPrograms } from 'apiSdk/financial-programs';
import { FinancialProgramInterface } from 'interfaces/financial-program';

import { FinancialPolicyListPage as ShowCaseList } from 'pages/financial-policies';

function DashboardPage() {
  const { session } = useSession();

  const budgetStat = useSWR<PaginatedInterface<BudgetInterface>>(
    () => `/budgets`,
    () => getBudgets({ limit: 1, offset: 0 }),
  );

  const financialPolicyStat = useSWR<PaginatedInterface<FinancialPolicyInterface>>(
    () => `/financial-policies`,
    () => getFinancialPolicies({ limit: 1, offset: 0 }),
  );

  const financialProgramStat = useSWR<PaginatedInterface<FinancialProgramInterface>>(
    () => `/financial-programs`,
    () => getFinancialPrograms({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Budgets',
      result: budgetStat?.data?.totalCount,
      isLoading: budgetStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Financial Policies',
      result: financialPolicyStat?.data?.totalCount,
      isLoading: financialPolicyStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Financial Programs',
      result: financialProgramStat?.data?.totalCount,
      isLoading: financialProgramStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Financial Policies
                </Text>
                <NextLink href={`financial-policies`} passHref legacyBehavior>
                  <Button
