...
import { getFinancialProcedureById } from 'apiSdk/financial-procedures';
import { FinancialProcedureInterface } from 'interfaces/financial-procedure';

function FinancialProcedureViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<FinancialProcedureInterface>(
    () => (id ? `/financial-procedures/${id}` : null),
    () =>
      getFinancialProcedureById(id, {
        relations: ['company'],
      }),
  );
      ...
      breadcrumbs={
        <Breadcrumbs
          items={[
            ...
            {
              label: 'Financial Procedure Details',
              isCurrent: true,
            },
          ]}
        />
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                <Box>
                  ...
                  >
                    Financial Procedure Details
                  </Text>
                </Box>
                {hasAccess('financial_procedure', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/financial-procedures/edit/${id}`} passHref legacyBehavior>
                    <Button
                ...
                <FormListItem
                  ...
                  text={
                    data?.implementation_date
                      ? format(parseISO(data?.implementation_date as unknown as string), 'dd-MM-yyyy')
                      : ''
                  }
                />
